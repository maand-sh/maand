	...
	for deploymentSeq := 0; deploymentSeq <= maxDeploymentSequence; deploymentSeq++ {
		...
		for _, job := range selectedJobs {
			...
			go func(tJob string) {
				...
				err := executeJobCommand(tJob, target, healthCheck, workersFilter, dockerClient)
				if err != nil {
					log.Fatal(err)
				}
...
func executeJobCommand(job string, target string, healthCheck bool, workersFilter []string, dockerClient *bucket.DockerClient) error {
	db, err := data.GetDatabase(true)
	if err != nil {
		return bucket.DatabaseError(err)
	}
	...
	if len(commands) > 0 {
		for _, command := range commands {
			err = jobcommand.JobCommand(tx, dockerClient, job, command, "job_control", len(allocatedWorkers), true, []string{})
			if err != nil {
				return err
			}
		}
	...
	for i := 0; i < workerCount; i += parallelBatchCount {
		...
		for j := range batchSize {
			...
			go func(tWorkerIP string) {
				...
				err = worker.ExecuteCommand(dockerClient, tWorkerIP, []string{fmt.Sprintf("python3 /opt/worker/%s/bin/runner.py %s %s --jobs %s", bucketID, bucketID, target, job)}, nil)
				if err != nil {
