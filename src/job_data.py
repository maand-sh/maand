...
def setup_job_database(cursor):
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS job_db.job (job_id TEXT PRIMARY KEY, name TEXT, version TEXT, min_memory_mb TEXT, max_memory_mb TEXT, min_cpu TEXT, max_cpu TEXT, certs_md5_hash TEXT, deployment_seq INT)")
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS job_db.job_commands (job_id TEXT, job_name TEXT, name TEXT, executed_on TEXT, depend_on_job TEXT, depend_on_command TEXT, depend_on_config TEXT)")
...
def get_job_certs_config(cursor, job):
    cursor.execute(
        "SELECT jc.name, jc.pkcs8, jc.subject FROM job_db.job_certs jc JOIN job j ON j.job_id = jc.job_id WHERE j.name = ?",
        (job,))
    rows = cursor.fetchall()
...
def get_job_commands(cursor, job, event):
    cursor.execute(
        "SELECT DISTINCT name as command_name FROM job_db.job_commands WHERE job_name = ? AND executed_on = ?",
        (job, event,))
    rows = cursor.fetchall()
    commands = []
...
def copy_job(cursor, name, agent_dir):
    cursor.execute(
        "SELECT path, content, isdir FROM job_db.job_files WHERE job_id = (SELECT job_id FROM job_db.job WHERE name = ?) AND path NOT LIKE ? ORDER BY isdir DESC",
...
def copy_job_modules(cursor, job):
    cursor.execute(
        "SELECT path, content, isdir FROM job_db.job_files WHERE job_id = (SELECT job_id FROM job_db.job WHERE name = ?) AND path like ? ORDER BY isdir DESC",
        (job, f"{job}/_modules%"))
    rows = cursor.fetchall()
