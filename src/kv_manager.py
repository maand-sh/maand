...
def put(cursor, namespace, key, value, ttl=-1):
    ...
    if row:
        ...
        if deleted == 0 and current_value == value:
            return
    cursor.execute('INSERT INTO kv_db.key_value (key, value, namespace, version, ttl, created_date, deleted) VALUES (?, ?, ?, ?, ?, ?, ?)', (key, value, namespace, version + 1, ttl, get_global_unix_epoch(), 0,))

def get(cursor, namespace, key):
    cursor.execute('SELECT value FROM kv_db.key_value WHERE namespace = ? AND key = ? AND version = (SELECT max(version) FROM kv_db.key_value WHERE namespace = ? AND key = ?) AND deleted = 0', (namespace, key, namespace, key))
    row = cursor.fetchone()
    return row[0] if row else None

def delete(cursor, namespace, key):
    cursor.execute('INSERT INTO kv_db.key_value (key, value, namespace, version, ttl, created_date, deleted) SELECT key, value, namespace, max(version) + 1 as version, ttl, created_date, 1 FROM kv_db.key_value WHERE namespace = ? AND key = ? GROUP BY key, namespace', (namespace, key,))

def get_keys(cursor, namespace):
    cursor.execute('SELECT key FROM (SELECT namespace, key, max(version), deleted, created_date FROM kv_db.key_value group by key, namespace) t WHERE namespace = ? AND deleted = 0',(namespace, ))
...
def gc(cursor, max_days):
    ...
    for row in rows:
        ...
        if version < 1:
            continue
        cursor.execute("DELETE FROM kv_db.key_value WHERE namespace = ? AND key = ? AND version = ?", (namespace, key, version))
