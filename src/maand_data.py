...
def get_new_allocations(cursor, job):
    cursor.execute("SELECT a.agent_ip FROM agent_jobs aj JOIN agent a ON a.agent_id = aj.agent_id WHERE aj.previous_md5_hash IS NULL AND aj.job = ?", (job,))
    rows = cursor.fetchall()
    return [row[0] for row in rows]


def get_changed_allocations(cursor, job):
    cursor.execute("SELECT a.agent_ip FROM agent_jobs aj JOIN agent a ON a.agent_id = aj.agent_id WHERE aj.previous_md5_hash IS NOT NULL AND aj.previous_md5_hash != aj.current_md5_hash AND aj.job = ?", (job,))
    rows = cursor.fetchall()
    return [row[0] for row in rows]
