    ...
    for agent_ip in agents:
        for job in jobs:
            ...
            for cert in job_certs:
                ...
                certs_keys.append(f"certs/{name}.key")
                certs_keys.append(f"certs/{name}.crt")
                certs_keys.append(f"certs/{name}.pem")
            ...
            all_certs = kv_manager.get_keys(cursor, namespace)
            missing_certs = set(all_certs) - set(certs_keys)
            for cert in missing_certs:
                kv_manager.delete(cursor, namespace, cert)
