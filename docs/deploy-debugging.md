...
Dry-run **stages** locally and refreshes plan hashes; it does **not** change workers or commit hash promotion. It may not simulate **stop** of removed/disabled allocations (see [deploy.md](./deploy.md)).
...
| `disabled` | Allocation disabled; stopped; catalog current | Re-enable via [disabled.md](./disabled.md) |
...
| Template panic | Allowed namespaces only — see [templates.md](./templates.md) |

### Disabled / re-enable surprises

| Symptom | Fix |
|---------|-----|
| Job stopped after disable | Expected; see [disabled.md](./disabled.md) |
...
Removed rows: hash cleared on deploy; worker **`jobs/<job>/`** tree deleted on **gc**. See [gc.md](./gc.md).
...
- [deploy.md](./deploy.md) — pipeline and failure table
- [disabled.md](./disabled.md) — disable and re-enable
- [rolling-upgrade.md](./rolling-upgrade.md) — **`update_parallel_count`** and reboot patterns
- [health-check.md](./health-check.md) — **`--update-hash`**
- [job-command.md](./job-command.md) — hook debugging
- [tutorials/day-2-operations.md](./tutorials/day-2-operations.md) — operations checklist
