...
| `resources.memory` / `cpu` | Min/max bounds; actual reservation from `bucket.jobs.conf` — see [resources-and-placement.md](./resources-and-placement.md) |
...
Full reference: [jobs-and-dependencies.md](./jobs-and-dependencies.md) (demand graph, max-depth rule, examples, inspect commands).

Details in [build.md](./build.md#deployment-sequence-deployment_seq) and [deploy.md](./deploy.md#deployment-sequence).
...
Job commands talk to maand’s **runtime HTTP API** and **KV store** on the CLI host; they receive a per-allocation workspace under `tmp/`. See [job-command.md](./job-command.md).
