...
| **Health** | Built-in TCP/HTTP/SSH manifest probes and/or custom **`health_check`** commands (probes first); **`--wait`**, **`--update-hash`** to trigger redeploy — see [health-check.md](./health-check.md) |
| **Prometheus** | Per-job **`_prometheus/`** scrape configs, alerts, runbooks; build aggregates to KV; prometheus job renders via **`{{ scrapeConfigs }}`** — see [prometheus.md](./prometheus.md) |
