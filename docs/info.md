...
Use **`maand cat workers`**, **`maand cat jobs`**, and **`maand cat allocations`** for detailed tables. For rollout state per allocation, use **`maand cat hashes`** — see [deploy-debugging.md](./deploy-debugging.md).
