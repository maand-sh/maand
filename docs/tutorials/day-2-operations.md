...
After [getting started](./getting-started.md), use these patterns for everyday cluster operations. Assumes a working bucket with deployed jobs.
...
Add **`workspace/jobs/api/_modules/command_health.py`** (see [job-command.md](../job-command.md)).
...
| Upgrade script needs old/new release | Read `CURRENT_VERSION` / `NEW_VERSION` in Makefile or job command env — [deploy.md](../deploy.md#allocation-version-tracking) |

Concept reference: [concepts.md](../concepts.md)
