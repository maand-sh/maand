...
Full rules: [manifest.md](../manifest.md#version) · [deployment-sequence.md](../deployment-sequence.md#build-validation)
...
How-to: [disable-and-drain.md](../../guides/disable-and-drain.md).
...
Assigned numbers are stored in **`job_ports`** and exposed in KV namespace **`maand/bucket`** (key `<port_name>`). Maand-provisioned ports keep the same number across rebuilds while it remains inside **`port_min`–`port_max`**. Changing a port from a **fixed integer** to **`{}`** reassigns from the pool when the stored number is outside that range (for example fixed `9500` → `{}` with default pool `30000`–`39999`). Fixed ports always follow the manifest value.
