...
func setupCatCatalogBucket(t *testing.T) {
	...
	_, err = tx.Exec(`
		...
		INSERT INTO worker_tags (worker_id, key, value) VALUES ('w1', 'zone', 'a');
...
func TestCatCatalogCommands(t *testing.T) {
	...
	workersOut := captureStdout(t, func() {
		require.NoError(t, Workers())
	})
	assert.Contains(t, workersOut, "ZONE")
	assert.Contains(t, workersOut, "a")

	allocationsOut := captureStdout(t, func() {
		require.NoError(t, Allocations("api", "10.0.0.1"))
	})
	assert.Contains(t, allocationsOut, "ZONE")
	assert.Contains(t, allocationsOut, "a")
