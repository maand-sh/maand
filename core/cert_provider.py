...
def generate_ca_private():
    command_manager.command_local(
        f"openssl genrsa -out {const.SECRETS_PATH}/ca.key 4096"
    )


def generate_ca_public(common_name, ttl):
    command_manager.command_local(
        f"openssl req -new -x509 -sha256 -days {ttl} -subj '/CN={common_name}' -key "
...
def generate_site_csr(name, subj, path):
    command_manager.command_local(
        f"openssl req -new -sha256 -subj '{subj}' -key {path}/{name}.key "
        f"-out {path}/{name}.csr"
    )
...
def generate_private_pem_pkcs_8(name, path):
    command_manager.command_local(
        f"openssl pkcs8 -inform PEM -outform PEM -in {path}/{name}.key -topk8 -nocrypt "
...
def generate_site_public(name, san, ttl, path):
    ...
    command_manager.command_local(
        f"openssl x509 -req -sha256 -days {ttl} -in {path}/{name}.csr "
