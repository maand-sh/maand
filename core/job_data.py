...
def get_jobs(cursor, deployment_seq=-1):
    if deployment_seq == -1:
        ...
        cursor.execute("SELECT name FROM job WHERE deployment_seq = ?", (deployment_seq,))
    rows = cursor.fetchall()
    return [row[0] for row in rows]


def get_max_deployment_seq(cursor):
    cursor.execute("SELECT ifnull(max(deployment_seq), 0) AS max_deployment_seq FROM job")
    row = cursor.fetchone()
    return row[0]
